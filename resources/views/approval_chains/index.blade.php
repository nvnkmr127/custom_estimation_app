<x-app-layout>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if(session('error'))
            <div x-data="{ show: true }" x-show="show" x-transition.opacity
                class="mb-8 rounded-lg bg-red-50 p-4 border border-red-100 shadow-sm relative">
                <div class="flex">
                    <div class="flex-shrink-0">
        ...
        <div class="grid grid-cols-1 gap-8">
            @forelse($chains as $chain)
                <div
                    class="group relative bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl hover:shadow-indigo-500/10 transition-all duration-300 overflow-hidden">
                    <!-- Status Bar -->
                    <div
                        class="absolute top-0 left-0 w-1 h-full {{ $chain->is_active ? 'bg-gradient-to-b from-green-400 to-emerald-500' : 'bg-gray-200' }}">
                    </div>
                    ...
                    <div class="p-6 sm:p-8">
                        <div class="sm:flex sm:items-center sm:justify-between">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-4">
                                    <h2
                                        class="text-xl font-bold text-gray-900 truncate group-hover:text-indigo-600 transition-colors">
                                        {{ $chain->name }}
                                    </h2>
                                    <span
                                        class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $chain->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                                        @if($chain->is_active)
                            ...
                            <!-- Actions -->
                            <div
                                class="mt-4 sm:mt-0 flex items-center space-x-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-x-4 group-hover:translate-x-0">
                                <a href="{{ route('approval-chains.edit', $chain) }}"
                                    class="text-gray-400 hover:text-indigo-600 bg-gray-50 hover:bg-indigo-50 p-2 rounded-full transition-colors">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                    </svg>
                                </a>
                                <form action="{{ route('approval-chains.destroy', $chain) }}" method="POST"
                                    onsubmit="return confirm('Are you sure you want to delete this approval chain?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="text-gray-400 hover:text-red-600 bg-gray-50 hover:bg-red-50 p-2 rounded-full transition-colors">
                                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-gray-100 pt-6">
                            ...
                            <div class="flex items-center gap-3">
                                <div
                                    class="flex-shrink-0 h-10 w-10 rounded-lg bg-purple-50 flex items-center justify-center text-purple-600">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="flex items-center gap-3">
                                <div
                                    class="flex-shrink-0 h-10 w-10 rounded-lg bg-amber-50 flex items-center justify-center text-amber-600">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <!-- Visual Flow -->
                        <div class="mt-8">
                            <h4 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Approval Sequence
                            </h4>
                                ...
                                @forelse($chain->steps as $step)
                                    <div class="flex items-center">
                                        <div class="relative flex items-center group/tooltip">
                                            <div
                                                class="flex items-center justify-center w-8 h-8 rounded-full bg-indigo-100 text-indigo-600 text-xs font-bold border-2 border-white ring-1 ring-indigo-50 shadow-sm relative z-10 transition-transform hover:scale-110">
                                                {{ $step->order }}
                                            </div>
                                            <div
                                                class="hidden sm:block absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover/tooltip:opacity-100 transition-opacity whitespace-nowrap z-20 pointer-events-none">
                                            ...
                                            <span
                                                class="ml-2 text-sm font-medium text-gray-700">{{ Str::limit(optional($step->user)->name ?? 'Unknown User', 15) }}</span>
                                        </div>
                                        @if(!$loop->last)
                    ...
                    <a href="{{ route('approval-chains.show', $chain) }}"
                        class="block px-6 py-3 bg-gray-50 border-t border-gray-100 text-center text-sm font-medium text-indigo-600 hover:text-indigo-700 hover:bg-gray-100 transition-colors">
            ...
            @empty
                <div class="text-center py-20 bg-white rounded-3xl shadow-sm border border-dashed border-gray-300">
                    ...
                    <p class="mt-2 text-sm text-gray-500 max-w-sm mx-auto">Get started by building your first approval
                        workflow to streamline your estimation process.</p>
                    <div class="mt-8">
                        <a href="{{ route('approval-chains.create') }}"
                            class="inline-flex items-center rounded-full bg-indigo-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-indigo-500/30 hover:bg-indigo-500 hover:shadow-indigo-500/50">
