    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10"
        ...
        <div class="mb-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div class="space-y-1">
                    ...
                    <ol role="list" class="flex items-center space-x-2">
                        <li>
                            <a href="{{ route('templates.index') }}"
                                class="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-indigo-600 transition-colors">Templates</a>
                        </li>
                        <li class="flex items-center space-x-2">
                            ...
                            <span
                                class="text-[10px] font-black uppercase tracking-widest text-slate-900">{{ isset($template) ? 'Edit' : 'Create' }}</span>
                        </li>
                    </ol>
                </nav>
                ...
                <p class="text-slate-500 font-medium">Define a reusable room configuration with pre-selected products
                    and units.</p>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="submit" form="template-form"
                    class="inline-flex items-center px-8 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-bold shadow-md shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95">
                    <svg class="mr-2 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
        ...
        <form id="template-form"
            ...
            <div class="grid grid-cols-1 gap-10 lg:grid-cols-3 items-start">
                ...
                <div class="lg:col-span-1 space-y-8">
                    <!-- Basic Stats Card (Visual Flair) -->
                    <div
                        class="bg-indigo-600 rounded-3xl p-8 shadow-xl shadow-indigo-100 flex items-center justify-between overflow-hidden relative">
                        <div class="relative z-10">
                            <div class="text-indigo-200 text-[10px] font-black uppercase tracking-widest mb-1">Items
                                Included</div>
                            <div class="text-4xl font-black text-white" x-text="items.length">0</div>
                        </div>
                        <div class="opacity-10 absolute -right-4 -bottom-4">
                            <svg class="h-32 w-32 text-white" fill="currentColor" viewBox="0 0 24 24">
                                <path
                                    d="M19 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zM9 17H7v-7h2v7zm4 0h-2V7h2v10zm4 0h-2v-4h2v4z" />
                            </svg>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-3xl p-8 shadow-xl shadow-slate-200/40 border border-slate-100">
                        <h2 class="text-xs font-black text-indigo-600 uppercase tracking-widest mb-8">General
                            Information</h2>

                        <div class="space-y-6">
                            <div>
                                <label for="name"
                                    class="block text-[11px] font-black uppercase tracking-widest text-slate-400 mb-2">Template
                                    Name <span class="text-rose-500">*</span></label>
                                <input type="text" name="name" id="name" required
                                    value="{{ old('name', $template->name ?? '') }}"
                                    placeholder="e.g. Master Bedroom Standard"
                                    class="block w-full rounded-2xl border-slate-200 py-3 px-4 shadow-sm focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition-all sm:text-sm">
                            </div>

                            <div>
                                <label for="description"
                                    class="block text-[11px] font-black uppercase tracking-widest text-slate-400 mb-2">Internal
                                    Notes</label>
                                <textarea name="description" id="description" rows="4"
                                    placeholder="Brief explanation of this room setup..."
                                    class="block w-full rounded-2xl border-slate-200 py-3 px-4 shadow-sm focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition-all sm:text-sm">{{ old('description', $template->description ?? '') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-3xl p-8 shadow-xl shadow-slate-200/40 border border-slate-100">
                        <div class="flex items-center justify-between mb-2">
                            <h2 class="text-xs font-black text-indigo-600 uppercase tracking-widest">Allowed Units</h2>
                            <template x-if="allowedUnitTypes.length > 0">
                                <button type="button" @click="allowedUnitTypes = []"
                                    class="text-[10px] font-black text-rose-500 uppercase tracking-widest hover:text-rose-700">Clear</button>
                            </template>
                        </div>
                        <p class="text-xs text-slate-400 font-medium mb-8 leading-relaxed">Limit which unit types can be
                            used for items in this template (e.g. Area, Length).</p>

                        <div class="space-y-4">
                            <template x-for="unitType in unitTypes" :key="unitType.id">
                                <label
                                    class="flex items-center p-3 rounded-2xl border border-slate-100 hover:border-indigo-100 hover:bg-indigo-50/30 transition-all cursor-pointer group">
                                    <div class="flex items-center h-5">
                                        <input type="checkbox" :value="String(unitType.id)" x-model="allowedUnitTypes"
                                            class="h-5 w-5 rounded-lg border-slate-300 text-indigo-600 focus:ring-indigo-600 cursor-pointer shadow-sm">
                                    </div>
                                    <div class="ml-4 flex-1">
                                        <div class="flex flex-col">
                                            ...
                                            <span
                                                ...
                                                x-text="unitType.units.join(' • ')"></span>
                                        </div>
                                    </div>
                                </label>
                            </template>
                ...
                <!-- Right Column: Room Items -->
                <div class="lg:col-span-2 space-y-8">
                    <div
                        class="bg-white rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100 overflow-hidden">
                        <div
                            class="px-8 py-8 border-b border-slate-50 flex flex-wrap items-center justify-between gap-6 bg-slate-50/30">
                            <div>
                                <h3 class="text-xl font-black text-slate-900 tracking-tight">Template Items</h3>
                                <p class="mt-1 text-sm text-slate-500 font-medium tracking-tight">Assign products and
                                    configure their default quantities.</p>
                            </div>
                            <button type="button" @click="openProductPicker()"
                                class="inline-flex items-center px-6 py-3 rounded-2xl bg-slate-900 text-white text-sm font-black shadow-lg shadow-slate-200 hover:bg-slate-800 transition-all active:scale-95 group">
                            ...
                            <table class="min-w-full divide-y divide-slate-100">
                                <thead>
                                    <tr class="bg-slate-50/50">
                                        <th
                                            class="px-8 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest w-16">
                                            Preview</th>
                                        <th
                                            class="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                            Product Details</th>
                                        <th
                                            class="px-6 py-4 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest w-24">
                                            Qty</th>
                                        <th
                                            class="px-6 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest w-48">
                                            Unit Configuration</th>
                                        <th
                                            class="px-6 py-4 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest w-32">
                                            Base Price</th>
                                        <th
                                            class="px-8 py-4 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest w-16">
                                        </th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr
                                            ...
                                            <td class="px-6 py-6">
                                                ...
                                                <div class="flex flex-col space-y-1">
                                                    <input type="text" :name="`items[${index}][item_name]`"
                                                        ...
                                                        class="block w-full border-0 p-0 text-sm font-black text-slate-900 placeholder:text-slate-300 focus:ring-0 bg-transparent group-hover:text-indigo-600 transition-colors"
                                                        placeholder="Item Name">

                                                    <input type="text" :name="`items[${index}][description]`"
                                                        x-model="item.description"
                                                        class="block w-full border-0 p-0 text-xs text-slate-400 font-bold placeholder:text-slate-200 focus:ring-0 bg-transparent"
                                                        placeholder="Add secondary details...">
                                                </div>
                                                ...
                                                <template x-if="item.options && item.options.length > 0">
                                                    <div class="flex flex-wrap gap-2 mt-3">
                                                        <template x-for="(opt, optIndex) in item.options"
                                                            :key="'opt-'+optIndex">
                                                            <div
                                                                class="inline-flex items-center px-2 py-0.5 rounded-lg bg-indigo-50 text-[10px] font-black text-indigo-600 border border-indigo-100 uppercase tracking-tight">
                                            ...
                                            <td class="px-6 py-6">
                                                <div class="flex flex-col items-center">
                                                    <input type="number" step="0.01" :name="`items[${index}][quantity]`"
                                                        ...
                                                        class="block w-16 rounded-xl border-transparent bg-slate-50/50 py-2 text-center text-sm font-black text-slate-900 focus:ring-2 focus:ring-indigo-600 focus:bg-white transition-all shadow-sm">
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-6">
                                                <div class="flex flex-col gap-2">
                                                    <!-- Simplified UI for Unit Assignment -->
                                                    <div class="space-y-2">
                                                        <div class="relative group/sel">
                                                            <select x-model="item.unit_type_id"
                                                                @change="onUnitTypeChange(item)"
                                                                class="block w-full rounded-xl border-slate-100 bg-slate-50/80 py-2 px-3 text-[10px] font-black text-slate-500 uppercase tracking-widest focus:ring-2 focus:ring-indigo-600 focus:bg-white transition-all appearance-none cursor-pointer hover:shadow-sm">
                                                            ...
                                                            <div
                                                                class="absolute inset-y-0 right-3 flex items-center pointer-events-none text-slate-300 group-hover/sel:text-indigo-400">
                                                        ...
                                                        <div class="relative">
                                                            <template x-if="item.unit_type_id">
                                                                <div class="relative">
                                                                    <select x-model="item.unit_type"
                                                                        ...
                                                                        class="block w-full rounded-xl border-indigo-100 bg-indigo-50/50 py-2 px-3 text-[11px] font-black text-indigo-600 focus:ring-2 focus:ring-indigo-600 focus:bg-white transition-all appearance-none cursor-pointer text-center shadow-sm">
                                                                        <template
                                                                    ...
                                                                    <div
                                                                        class="absolute inset-y-0 right-3 flex items-center pointer-events-none text-indigo-300">
                                                            ...
                                                            <template x-if="!item.unit_type_id">
                                                                <input type="text" x-model="item.unit_type"
                                                                    :name="`items[${index}][unit_type]`"
                                                                    placeholder="e.g. nos"
                                                                    class="block w-full rounded-xl border-slate-100 bg-white py-2 px-3 text-[11px] font-black text-slate-900 text-center focus:ring-2 focus:ring-indigo-600 shadow-sm placeholder:text-slate-200">
                                                            </template>
                                                        </div>
                                                    </div>
                                            ...
                                            <td class="px-6 py-6 text-right">
                                                <div class="relative inline-block w-28">
                                                    <span
                                                        class="absolute inset-y-0 left-3 flex items-center text-slate-300 font-black text-xs">₹</span>
                                                    <input type="number" step="0.01"
                                                        :name="`items[${index}][unit_price]`" x-model="item.unit_price"
                                                        class="block w-full rounded-xl border-transparent bg-slate-50/50 py-2 pl-7 pr-3 text-right text-sm font-black text-slate-900 focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all shadow-sm">
                                                </div>
                                            </td>
                                    ...
                                    <tr x-show="items.length === 0">
                                        <td colspan="6" class="px-8 py-20 text-center">
                                            <div class="max-w-xs mx-auto flex flex-col items-center">
                                                ...
                                                <h4 class="text-base font-black text-slate-900 mb-2">Build your template
                                                </h4>
                                                ...
                                                <button type="button" @click="openProductPicker()"
                                                    class="px-8 py-3 rounded-2xl bg-indigo-600 text-white text-xs font-black uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95">
                                                    Open Library
                                                </button>
                                            </div>
                                        </td>
                        ...
                        <div class="px-8 py-6 bg-slate-50/30 border-t border-slate-50 text-center">
                            <button type="button" @click="openProductPicker()"
                                class="text-xs font-black text-slate-400 hover:text-indigo-600 transition-colors uppercase tracking-widest">
                ...
                <div class="flex min-h-full items-center justify-center p-4">
                    <div x-show="productPicker.isOpen" x-transition:enter="ease-out duration-300"
                        ...
                        <div class="p-8 pb-4 flex items-center justify-between border-b border-slate-50">
                            <div>
                                <h3 class="text-2xl font-black text-slate-900 tracking-tight">Product Library</h3>
                        ...
                        <div class="p-8">
                            ...
                            <div class="relative mb-8">
                                ...
                                <input type="text" x-model="productPicker.search"
                                    class="block w-full rounded-2xl border-slate-200 bg-slate-50/50 py-4 pl-12 pr-4 text-slate-900 placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition-all sm:text-sm shadow-inner"
                            ...
                            <div class="max-h-[50vh] overflow-y-auto space-y-3 pr-2 scrollbar-none">
                                <template x-for="product in filteredProducts" :key="product.id">
                                    <button type="button" @click="selectProduct(product)"
                                        class="w-full text-left p-4 rounded-3xl bg-white border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50/30 hover:-translate-y-1 hover:shadow-lg transition-all flex justify-between items-center group">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="h-16 w-16 rounded-2xl bg-slate-100 border border-slate-200/50 flex-shrink-0 overflow-hidden group-hover:scale-105 transition-transform">
                                            ...
                                            <div>
                                                <div class="font-black text-slate-900 group-hover:text-indigo-600 transition-colors"
                                        ...
                                        <div class="text-right">
                                            <div class="text-lg font-black text-slate-900">₹<span
                                ...
                                <div x-show="filteredProducts.length === 0"
                                    ...
                                    <p class="font-black text-xs uppercase tracking-widest">No matching products</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="p-8 pt-4 bg-slate-50/50 flex flex-col gap-3">
                            <button type="button" @click="addCustomItem()"
                                class="w-full flex items-center justify-center gap-3 rounded-2xl bg-white px-6 py-4 text-xs font-black text-slate-600 uppercase tracking-widest hover:bg-slate-100 border border-slate-200 transition-all shadow-sm active:scale-[0.98]">
                        ...
                        <div x-show="configModal.product">
                                        ...
                                        <div class="grid grid-cols-2 gap-3">
                                            <template x-for="value in option.values" :key="'val-'+value.id">
                                                <div @click="configModal.options[option.id] = value.id"
                                                    ...
                                                    <span class="text-sm font-black text-slate-900 transition-colors"
                                                        :class="{'text-indigo-700': configModal.options[option.id] === value.id}"
                                                        x-text="value.value"></span>

                                                    <span x-show="value.price_adjustment != 0"
                                                        class="mt-1 text-[10px] font-black uppercase tracking-tighter transition-colors"
                            ...
                            <div class="p-8 bg-slate-50 border-t border-slate-100">
                                <button type="button" @click="confirmConfig()"
                                    class="w-full py-4 rounded-2xl bg-indigo-600 text-white text-sm font-black shadow-xl shadow-indigo-200 hover:bg-indigo-700 transition-all active:scale-95 flex items-center justify-center gap-3">
    ...
    <script>
        function templateForm(products, initialItems, unitTypes) {
            return {
                ...
                addCustomItem() {
                    this.items.push({ product_id: null, item_name: 'Custom Product', description: '', quantity: 1, unit_type: 'nos', unit_type_id: '', unit_price: 0, options: [] });
                    this.productPicker.isOpen = false;
                },
