<x-app-layout>
    <div class="max-w-4xl mx-auto">
        <div class="mb-8">
            ...
            <h1 class="mt-2 text-2xl font-bold tracking-tight text-slate-900">Create Item Package</h1>
            <p class="mt-1 text-sm text-slate-500">Create a new package of items to quick-add to estimates.</p>
        </div>

        <form action="{{ route('packages.store') }}" method="POST" class="space-y-8" x-data="{ 
                  isSubmitting: false,
                  items: [{item_name: '', quantity: 1, unit_type: 'pcs', unit_price: 0}]
              }">
            @csrf
                    ...
                    <div class="col-span-full">
                        ...
                        <div class="mt-2">
                            <textarea name="description" id="description" rows="3"
                                class="block w-full rounded-lg border-slate-300 py-1.5 text-slate-900 shadow-sm focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">{{ old('description') }}</textarea>
                        </div>
                    </div>
                </div>
            </x-card>
                        ...
                        <div class="flex gap-4 items-start p-4 bg-slate-50 rounded-xl border border-slate-200">
                            ...
                            <div class="pt-8">
                                <button type="button" @click="items.splice(index, 1)"
                                    class="text-rose-600 hover:text-rose-900" x-show="items.length > 1">
            ...
            <div class="flex items-center justify-end gap-x-6">
                ...
                <x-primary-button type="submit" class="px-8 py-2.5" x-bind:disabled="isSubmitting"
                    ...
                    <div class="flex items-center">
                        ...
                        <span x-text="isSubmitting ? 'Creating...' : 'Create Package'"></span>
                    </div>
                </x-primary-button>
            </div>
        </form>
