        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between px-8 py-10 gap-6">
            <div>
                ...
                <p class="mt-4 text-lg text-indigo-100 max-w-xl font-medium leading-relaxed">
                    Broadcast system events to your external services. Manage endpoints, monitor health, and debug delivery issues.
                </p>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-slate-100">
                <thead class="bg-white">
                    <tr>
                        <th scope="col" class="px-8 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-indigo-600 transition-colors" wire:click="sortBy('name')">
                            <div class="flex items-center gap-2">
                                Endpoint
                                @if($sortField === 'name')
                                    <svg class="h-3 w-3 {{ $sortDirection === 'asc' ? 'rotate-180' : '' }} transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                                    </svg>
                                @endif
                            </div>
                        </th>
                        <th scope="col" class="px-8 py-4 text-left text-xs font-bold text-slate-400 uppercase tracking-wider">Target URL</th>
                        <th scope="col" class="px-8 py-4 text-center text-xs font-bold text-slate-400 uppercase tracking-wider">Health</th>
                        <th scope="col" class="px-8 py-4 text-right text-xs font-bold text-slate-400 uppercase tracking-wider">Actions</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-slate-50/80 transition-colors group">
                            <td class="px-8 py-5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div class="text-sm font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">{{ $webhook->name }}</div>
                                        <div class="flex items-center gap-1.5 mt-0.5">
                                            @if($webhook->status === 'active')
                                                <span class="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-700 ring-1 ring-emerald-600/10">
                                                    <span class="h-1 w-1 rounded-full bg-emerald-500"></span> Active
                                                </span>
                                            @elseif($webhook->status === 'inactive')
                                                <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-600 ring-1 ring-slate-600/10">
                                                    <span class="h-1 w-1 rounded-full bg-slate-400"></span> Disabled
                                                </span>
                                            @else
                                                <span class="inline-flex items-center gap-1 rounded-full bg-rose-50 px-2 py-0.5 text-[10px] font-bold text-rose-700 ring-1 ring-rose-600/10">
                                                    <span class="h-1 w-1 rounded-full bg-rose-500"></span> {{ ucfirst(str_replace('_', ' ', $webhook->status)) }}
                                                </span>
                                            @endif
                                            <span class="text-[10px] text-slate-400 font-medium">{{ count($webhook->events ?? []) }} event types</span>
                                        </div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-5">
                                ...
                                @if($webhook->latestDelivery)
                                    <div class="mt-1 text-[10px] text-slate-400">
                                        ...
                                        <span class="{{ $webhook->latestDelivery->status === 'success' ? 'text-emerald-500' : 'text-rose-500' }}">({{ ucfirst($webhook->latestDelivery->status) }})</span>
                                    </div>
                                @endif
                            </td>
                            <td class="px-8 py-5 text-center">
                                ...
                                <div class="inline-flex flex-col items-center">
                                    ...
                                    <span class="text-[10px] text-slate-400 mt-1 uppercase font-bold tracking-wider">Success Rate</span>
                                </div>
                            </td>
                            <td class="px-8 py-5 whitespace-nowrap text-right text-sm">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <a href="{{ route('admin.webhooks.logs.index', ['webhookId' => $webhook->id]) }}"
                                        class="p-2 rounded-xl hover:bg-slate-100 text-slate-400 hover:text-indigo-600 transition-all shadow-sm bg-white ring-1 ring-slate-100"
                                        title="View Logs">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 2v-6m-8 9h11a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v11a2 2 0 002 2z" /></svg>
                                    </a>
                        ...
                        <tr>
                            <td colspan="4" class="px-8 py-20 text-center text-slate-500">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="font-bold text-slate-400 uppercase tracking-widest text-xs">No webhooks found</p>
                                    <a href="{{ route('admin.webhooks.create') }}" class="mt-4 text-indigo-600 font-bold hover:underline">Create your first endpoint &rarr;</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
