    ...
    protected function renderItems($block, $items, $parentSection = null)
    {
        ...
        foreach ($items as $item) {
            ...
            $itemVars = [
                ...
                '{item_currency}' => $this->context ? $this->context->get('currency') : ($this->variables['currency'] ?? '$'),
                '{item_size}' => $item->size ?? '',
                ...
                'has_length' => (($item->length + 0) > 0) ? 1 : 0,
                'has_width' => (($item->width + 0) > 0) ? 1 : 0,
                'has_height' => (($item->height + 0) > 0) ? 1 : 0,
                'has_size' => (!empty($item->size)) ? 1 : 0,
                'has_dimensions' => (($item->length + 0) > 0 || ($item->width + 0) > 0 || ($item->height + 0) > 0) ? 1 : 0,
                'has_any_dimensions' => (($item->length + 0) > 0 || ($item->width + 0) > 0 || ($item->height + 0) > 0 || !empty($item->size)) ? 1 : 0,
